## VERSION OF DASHBOARD WITH NO PLOTLY - Removes interactive features with forest and bar plots
                 ...
                 tabPanel(title="Overall & Subpopulation Results",
                          fluidRow(
                            ...
                            box(width=5,title="Primary Results",solidHeader=TRUE,status="primary",align='right',
                                ...
                                plotOutput("pooled_plots")  %>% withSpinner(color="#a9a9a9"),
                          ...
                          fluidRow(
                            ...
                            tabBox(width=6,title="",id="cbsa_tabs",
                                   tabPanel("Forest/Bar Plot",
                                            ...
                                            plotOutput('forest_plot') %>% withSpinner(color="#a9a9a9")
                                   ),
                            ...
                            box(width=4,title="MSA-Specific Results",solidHeader = TRUE,status="primary",
                                selectInput(inputId="cbsa_choice",
                                            label="Select an MSA (below or in the interactive map to the left):",
...
server <- function(input, output,session) { 
  ...
  output$pooled_plots <- renderPlot({
    ...
    forest.pooled <- ggplot(data=data.forest[data.forest$variable == "Relative Risk (99th vs. MHP)", ], 
      ...
      geom_errorbar(width=0.1,position=pd,size=1.005) + theme_bw(base_size=15) + 
    ...
    bar.pooled <- ggplot(data=data[data$exposure == lab,], 
      ...
      geom_col(alpha=0.85,position=pd) +   geom_errorbar(width=.1,color='black',position=pd,size=1.005) + theme_bw(base_size=15) + 
    ...
    if (input$sub=="UHII") {
      grid.arrange(forest.pooled + theme(legend.position = c(0.175,0.125), legend.title=element_blank()),
                   bar.pooled + theme(legend.position = c(0.175,0.9), legend.title=element_blank()),ncol=2,nrow=1)
    } else {
      grid.arrange(forest.pooled + theme(legend.position = "none"),bar.pooled + theme(legend.position = "none"),ncol=2,nrow=1)
    }
  })
  ...
  output$forest_plot <- renderPlot({
    ...
    if (input$forest_variable == "rr.99") {
      ...
      x="cen";xmin="cen.low";xmax="cen.high";alpha.val=1
    ...
    if (input$forest_variable %in% c("an.heat","ar.heat")) {
      if (input$forest_type == "UHII") {
        forest.temp <- forest.temp + geom_col(position=dodge) + 
          geom_errorbarh(height=0.5,size=tck_size,color='black',position=dodge) + 
        ...
        forest.temp <- forest.temp + geom_col() + 
          geom_errorbarh(height=0.5,size=tck_size,color='black') + theme_bw(base_size=14) + 
          geom_vline(xintercept=xline, color='black', linetype='dashed', alpha=.5) +
          theme(axis.title.y = element_blank()) +
          xlab(xlab)
      }
      ...
      if (input$forest_type == "UHII") {
        forest.temp <- forest.temp + geom_point(position=dodge,size=3) + 
          geom_errorbarh(height=0.5,size=tck_size,position=dodge) + theme_bw(base_size=15) + 
          theme(axis.title.y = element_blank()) +
          xlab(xlab)
      } else {
        forest.temp <- forest.temp + geom_point(size=3) + 
          geom_errorbarh(height=0.5,size=tck_size) + theme_bw(base_size=14) + 
          theme(axis.title.y = element_blank()) +
          xlab(xlab)
      }
    ...
    if (input$forest_color%in%c("avg.range","avg.temp") && input$forest_type == "Primary") {
      ...
      forest.temp <- forest.temp + theme(legend.key.height = unit(3.5, "cm"))
      
    } else if ((input$forest_color%in%c("region","climate") && input$forest_type == "Primary" )|| input$forest_type == "UHII"){
      if (input$forest_variable %in% c("an.heat","ar.heat")) {
        forest.temp <-  forest.temp + scale_fill_manual(values = alpha(pal,alpha.val),name=color.lab) 
      } else {
        forest.temp <-  forest.temp + scale_colour_manual(values = pal,name=color.lab) 
        
      }
      forest.temp <- forest.temp + theme(legend.key.height = unit(1.5, "cm"))
      
    }
    
    if (input$forest_color != 'none') {
      forest.temp <- forest.temp + theme(legend.text=element_text(size=13))
    }
    
    plot <- forest.temp + xlim(xlim)
